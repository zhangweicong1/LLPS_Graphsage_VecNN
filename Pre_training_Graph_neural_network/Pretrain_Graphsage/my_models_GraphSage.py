...
class UnsupervisedLoss(object):
    ...
    def get_loss_sage(self, embeddings, nodes):
        ...
        # print(f"self.node_positive_pairs：{self.node_positive_pairs}")
        # print(f"self.node_negative_pairs: {self.node_negtive_pairs}")
        ...
        # print(nodes_score)
        # print(f"torch.cat(nodes_score, 0){torch.cat(nodes_score, 0)}")
    ...
    def get_loss_margin(self, embeddings, nodes):
        ...
        for node in self.node_positive_pairs:
            ...
            neighb_indexs = [node2index[x] for x in indexs[1]]
            pos_score = F.cosine_similarity(embeddings[node_indexs], embeddings[neighb_indexs])
            # raw_pos = F.cosine_similarity(embeddings[node_indexs],embeddings[neighb_indexs])
            # print(f"raw_pos mean={raw_pos.mean().item():.4f}, "f"min={raw_pos.min().item():.4f}, "f"max={raw_pos.max().item():.4f}")
            ...
            neighb_indexs = [node2index[x] for x in indexs[1]]
            neg_score = F.cosine_similarity(embeddings[node_indexs], embeddings[neighb_indexs])
            # raw_neg = F.cosine_similarity(embeddings[node_indexs],embeddings[neighb_indexs])
            # print(f"raw_neg mean={raw_neg.mean().item():.4f}, "f"min={raw_neg.min().item():.4f}, "f"max={raw_neg.max().item():.4f}")
            neg_score, _ = torch.max(torch.log(torch.sigmoid(neg_score)), 0)
            #print(f"pos={pos_score.item():.4f}, "f"neg={neg_score.item():.4f}, "f"margin_loss={(neg_score-pos_score+self.MARGIN).item():.4f}")
...
class GraphSage(nn.Module):
    ...
    def __init__(self, num_layers, input_size, out_size, datacenter, raw_features, adj_lists, device, gcn=False, agg_func='MEAN'):
    ...
    def _get_unique_neighs_list(self, nodes, num_sample=10):
        ...
        #print(to_neighs)
        # to_neighs = [self.adj_lists[node] if self.adj_lists[node] is not None else {} for node in nodes]
        # print(to_neighs)
        if not num_sample is None:
            _sample = random.sample
            samp_neighs = [_set(_sample(list(to_neigh), num_sample)) if len(to_neigh) >= num_sample else to_neigh for to_neigh
                           in to_neighs]
        else:
